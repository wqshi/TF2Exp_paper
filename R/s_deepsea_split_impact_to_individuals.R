...
if (batch_name == '462samples_sailfish'){
    deepsea_dir = '/homed/home/shi/expression_var/data/462samples_sailfish/deep_result/all/chrMerge2/evalue/'
}else{
    deepsea_dir = '/homed/home/shi/expression_var/data/800samples/deep_result/all/chr800/evalue/'
}
...
f_split_one_chromosom <- function(chr_str, deepsea_dir, batch_name, test_flag = FALSE){
    ...
    chr_impact = read.table(f_p('%s/%s.diff.gz', deepsea_dir, chr_str), sep = ',', header = T, )
    ...
    if (test_flag == TRUE){
        individual_vcfs_list=individual_vcfs_list[1:20]
    }
    ...
    for (individual_vcf in individual_vcfs_list){
        ...
        write.table(write_data[complete.cases(write_data),], gz1, sep = ',', row.names = FALSE, col.names = TRUE, quote = FALSE)
        close(gz1)
    }
}

f_split_one_chromosom(chr_str, deepsea_dir, batch_name, test_flag = TRUE)
