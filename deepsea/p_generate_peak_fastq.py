...
class chipseq_region():
    ...
    def bed_trim_binding_regions(self, peak_max_col = 9):
    ...
    def bed4_format_peak(self, other_col):
        #import ipdb; ipdb.set_trace()
        self.binding_df = pd.io.parsers.read_csv(self.peak_file, sep="\t", header=None).ix[:,range(3) + [other_col]]
...
def f_prepare_deepsea_fastq_based_on_vcf(peak_file, vcf_file, tmp_dir, debug = False):
    if f_judge_debug(debug):
        import ipdb; ipdb.set_trace()
    ...
    fastq_file = tf_region.bed_to_fastq(overlap_pd, tmp_dir, debug = False)
    return fastq_file
