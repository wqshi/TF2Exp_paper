...
if __doc__  is None:
    ...
    infilename = './examples/deepsea/example.vcf'
    outdir = 'outdir'
...
#for vcf input 
    ...
    try:
        ...
        print "Successfully copied input to working directory " + tempdir 
        try:
            check_call(["Rscript3  0_coor2fasta.R "+tempdir+'/infile.vcf '],shell=True)
        except:
            raise Exception('Vcf format error.')
        #retrieve 1100bp instead of 1000bp for supporting deletion variants (<100bp) 
        check_call(["python2.7 1_fasta2input.py  "+tempdir+"/infile.vcf.wt1100.fasta 1100"],shell=True) 
        print "Successfully converted to input format"
